...
const FilterWeather = () => {
    const { activeWeatherFilter, allWeatherFilters } = useSelector(
    ...
    const filterItems = allWeatherFilters.map((item) => {
        ...
        return (
            <FilterWeatherItem key={item.label} item={item} classes={classes} />
        )
    })

    return <ul className="weather-list-filter">{filterItems}</ul>
}
